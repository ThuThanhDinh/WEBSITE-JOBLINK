...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    //     toast.success("Logged in successfully!"); // Optional: for notifications
    //     navigate("/dashboard"); // Adjust the path as needed
    //   } else {
    //     const errorData = await response.json();
    //     // Handle error (e.g., show error message)
    //     toast.error(`Error: ${errorData.message}`); // Optional: for notifications
    //   }
    // } catch (error) {
    //   toast.error(`Error: ${error.message}`); // Optional: for notifications
    // }
  };
